...
@Component({
  templateUrl: 'app/component/etest/etest-start.component.html'+environment.fileVersion,
  directives: [ROUTER_DIRECTIVES],
  providers: [ElearningApi, QuizResult]
 // styleUrls: ['etest.component.css']
})
export class EtestStartComponent implements OnInit {
	...
	getQuizPage(){
		this.pages =[];
		this.elearingApi.getElearningQuizPageList(this.categoryId).toPromise().then((data)=>{
				data.forEach((v,i)=>{
	...
	startQuiz(index: number) {
		...
		if(index>0&&index <=this.pages.length) {
			if(this.quizResult.currR1<0||this.quizResult.currR2<0) {
	...
	chooseOption(postion:string,right:boolean,index:number){
		//console.log(right+" choose option"+postion);
		if(this.sleepTime) {
			return;
		}
		if(postion=='r1') {
